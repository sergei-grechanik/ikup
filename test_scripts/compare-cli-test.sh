...
if [ -d "$1" ]; then
    exec uv run --no-sync python -m ikup.testing.output_comparison "$1" ./data/cli-test-references
fi
...
exec uv run --no-sync python -m ikup.testing.output_comparison "$TEST_OUTPUT_FILE" "$REFERENCE_FILE"
